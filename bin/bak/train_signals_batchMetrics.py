...
batch_size = 4 # 8 # using 8 NVIDIA GeForce RTX A5000 (24GB) GPUs
warmup_epoches = 20
num_epoches = 50 # 100
...
for epoch in range(num_epoches):
    ...
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        with torch.set_grad_enabled(True):
            ...
            targets = torch.flatten( torch.log2(targets+1), start_dim=1 ) # flatten
            outs = torch.flatten( torch.log2(outs+1), start_dim=1 ) # flatten
            
            corr = metric(targets, outs)
            if torch.isnan(corr):
                corr = torch.nan_to_num(corr, nan=0.0)
                train_loader_size = train_loader_size - 1 
    ...
    epoch_corr = running_corr / train_loader_size 
    epoch_loss = running_loss / train_loader_size
    ...
    for idx, test_data in enumerate(val_loader, 0):
        ...
        # append target and output
        signals = torch.flatten( torch.log2(signals+1), start_dim=1 ) # reshape and transform
        outputs = torch.flatten( torch.log2(outputs+1), start_dim=1 ) # reshape and transform
        
        test_corr = metric(signals, outputs)
        if torch.isnan(test_corr):
            test_corr = torch.nan_to_num(test_corr, nan=0.0)
            val_loader_size = val_loader_size - 1 
        test_running_corr += test_corr.cpu().detach().numpy() 

    # calculate PCC statistics
    test_corr = test_running_corr / val_loader_size
    test_loss = test_running_loss / val_loader_size 
