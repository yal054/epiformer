...
batch_size = 4 # using 4 NVIDIA GeForce RTX 3090 GPUs
warmup_epoches = 10
...
dataset_list = [HDF5Dataset(f_path, preprocess=True, if_log2=False, which = celltype_index) for f_path in infile_list[0:2]]
...
def main(rank, world_size):
    ...
    #dataset_list = [HDF5Dataset(f_path, preprocess=True, if_log2=False, which = celltype_index) for f_path in infile_list[0:2]]
    #dataset = ConcatDataset(dataset_list)
    #train_loader = prepare(dataset, rank, world_size, batch_size=batch_size, pin_memory=False, num_workers=0)
    
    sampler = DistributedSampler(dataset)
    train_loader = DataLoader(dataset, batch_size=batch_size, num_workers=0, sampler=sampler)
    ...
    for epoch in range(num_epoches):
        ...
        for batch_idx, data in enumerate(train_loader, 0):
            ...
            with torch.set_grad_enabled(True):
                ...
                # print(loss)
                if penalty > 0:
                    q99 = torch.quantile(targets, 0.99)
                    idxy = targets < q99 # > q99 
    ...
    #        
    #        if penalty > 0:
    #            q99 = torch.quantile(signals, 0.99)
    #            test_idxy = signals < q99 # > q99
    ...
    #            epoch_loss, test_loss, epoch_corr, test_corr))
        print('Model_Loss: {:.4f} Model_Corr: {:.4f}'.format( epoch_loss, epoch_corr))
